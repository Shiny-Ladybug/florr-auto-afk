...
class ChPPOCRv3:
    ...
    def __init__(self, text_detector_module=None, enable_mkldnn=False):
        ...
        """
        self.character_dict_path = os.path.join(
            self.directory, 'assets', 'ppocr_keys_v1.txt')
        char_ops_params = {
            'character_type': 'ch',
        ...
        self.rec_pretrained_model_path = os.path.join(
            self.directory, 'inference_model', 'ppocrv3_rec')
        self.cls_pretrained_model_path = os.path.join(
            self.directory, 'inference_model', 'ppocr_cls')
    ...
    def text_detector_module(self):
        ...
        if not self._text_detector_module:
            self._text_detector_module = hub.Module(directory=os.path.join(os.path.abspath(os.path.join(
                os.path.dirname(__file__), '..')), 'ch_pp_ocrv3_det'),
    ...
    def recognize_text(self,
        ...
        boxes = [np.array(item['data']).astype(np.float32)
    ...
    def _classify_text(self, image_list, angle_classification_thresh=0.9):
        ...
        for beg_img_no in range(0, img_num, batch_num):
            ...
            cls_result = [(label_list[idx], prob_out[i, idx])
            ...
            for rno in range(len(cls_result)):
                ...
                if '180' in label and score > angle_classification_thresh:
                    img_list[indices[beg_img_no + rno]
                             ] = cv2.rotate(img_list[indices[beg_img_no + rno]], 1)
    ...
    def _recognize_text(self, img_list):
        ...
        for beg_img_no in range(0, img_num, batch_num):
            ...
            rec_result = self.char_ops.decode(
